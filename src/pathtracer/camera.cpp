...
Ray Camera::generate_ray(double x, double y) const {
  ...
  //
    double tanx = tan(0.5 * radians(hFov));
    double tany = tan(0.5 * radians(vFov));
    
    double x_pos = (x * 2 * tanx) - tanx;
    double y_pos = (y * 2 * tany) - tany;
    
    Vector3D cam_space = Vector3D(x_pos, y_pos, -1);
    
    cam_space = c2w * cam_space;
    
    Ray gen_ray = Ray(pos, cam_space.unit());
    gen_ray.min_t = nClip;
    gen_ray.max_t = fClip;
  return gen_ray;
}
